...
function renderCartDrawer() {
  ...
  document.getElementById('cart-subtotal').textContent = `$${cartSubtotal().toLocaleString()}`;
    ...
    return `
      <div class="cart-line" data-id="${p.id}">
        ...
        <div class="cart-line-info">
          <h4>${p.name}</h4>
          <span class="cart-line-price">$${p.price.toLocaleString()} / unit</span>
  ...
  document.getElementById('checkout-body').innerHTML = `
    ...
    <form id="checkout-form">
      ...
      <div class="order-summary">
        ${cart.map(l => `<div class="order-summary-line"><span>${l.product.name} × ${l.qty}</span><span>$${(l.product.price * l.qty).toLocaleString()}</span></div>`).join('')}
        <div class="order-summary-line"><span>Estimated installation support</span><span>$${installEstimate.toLocaleString()}</span></div>
        <div class="order-summary-line total"><span>Total</span><span>$${total.toLocaleString()}</span></div>
      </div>
