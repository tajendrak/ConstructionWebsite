...
// Used as the onerror fallback on <img> tags: if a real product photo hasn't
// been added to /images/<PRODUCT-ID>.jpg yet, swap in the SVG glyph instead
// of showing a broken-image icon. Returns an actual element (not a string)
// because it's called from `this.replaceWith(...)`.
...
const cart = [];              // in-memory quote list: [{ product, qty }, ...]
                               // see addToCart()/CART SYSTEM section for why
                               // this is in-memory rather than localStorage.
  ...
  grid.innerHTML = list.map(p => `
    <div class="product-card" data-id="${p.id}">
      <div class="product-media">
        ...
        <img src="images/${p.id}.svg" alt="${p.name}" loading="lazy"
             onerror="this.replaceWith(iconElement('${p.icon}'))">
      </div>
      <div class="product-body">
        ...
        <div class="product-foot">
          <span class="price">MYR ${p.price.toLocaleString()} <span>/ unit</span></span>
...
function openProductModal(productId) {
  ...
  document.getElementById('modal-media').innerHTML =
    `<img src="images/${p.id}.svg" alt="${p.name}" onerror="this.replaceWith(iconElement('${p.icon}'))">`;
  ...
  document.getElementById('modal-price').innerHTML =
    `MYR ${p.price.toLocaleString()} <span style="font-size:0.7rem;">/ unit</span>`;
...
/* ---------------------------------------------------------------------- */
/* 5. CART SYSTEM — drawer, quantities, subtotal, checkout (DEMO).         */
/* State is in-memory only, not localStorage: keeps this dependency-free  */
/* and avoids storage permission issues when the file is opened directly  */
/* (file://) instead of served. Swap in a real store/backend to go live.  */
/* ---------------------------------------------------------------------- */
...
function addToCart(productId) {
  ...
  updateCartBadge();
  renderCartDrawer();
  showToast(`Added "${p.name}" to quote list`);
}
...
function cartSubtotal() { return cart.reduce((sum, l) => sum + l.qty * l.product.price, 0); }
...
function renderCartDrawer() {
  ...
  document.getElementById('cart-subtotal').textContent = `MYR ${cartSubtotal().toLocaleString()}`;
  document.getElementById('checkout-btn').disabled = cart.length === 0;

  if (cart.length === 0) {
    container.innerHTML = `<p class="cart-empty">Your quote list is empty.<br>Add products from the catalog to get started.</p>`;
    return;
  }
    ...
    return `
      <div class="cart-line" data-id="${p.id}">
        ...
        <div class="cart-line-info">
          <h4>${p.name}</h4>
          <span class="cart-line-price">MYR ${p.price.toLocaleString()} / unit</span>
          ...
          <button class="cart-line-remove" data-id="${p.id}">Remove</button>
        </div>
      </div>`;
  }).join('');

  container.querySelectorAll('.qty-plus').forEach(b => b.addEventListener('click', () => changeQty(b.dataset.id, 1)));
  container.querySelectorAll('.qty-minus').forEach(b => b.addEventListener('click', () => changeQty(b.dataset.id, -1)));
  container.querySelectorAll('.cart-line-remove').forEach(b => b.addEventListener('click', () => removeFromCart(b.dataset.id)));
}
...
function renderCheckoutForm() {
  ...
  const installEstimate = Math.round(subtotal * 0.08); // demo estimate line only
  const total = subtotal + installEstimate;

  document.getElementById('checkout-body').innerHTML = `
    <h2 class="mt-0">Checkout</h2>
    <p>Confirm your details — this places a quote request, not a live payment.</p>
    <form id="checkout-form">
      <div class="form-grid">
        <div><label for="co-name">Full name</label><input id="co-name" required></div>
        <div><label for="co-email">Email</label><input id="co-email" type="email" required></div>
        <div class="full"><label for="co-company">Company</label><input id="co-company"></div>
        <div class="full"><label for="co-address">Delivery address</label><input id="co-address" required></div>
      </div>
      <div class="order-summary">
        ${cart.map(l => `<div class="order-summary-line"><span>${l.product.name} × ${l.qty}</span><span>MYR ${(l.product.price * l.qty).toLocaleString()}</span></div>`).join('')}
        <div class="order-summary-line"><span>Estimated installation support</span><span>MYR ${installEstimate.toLocaleString()}</span></div>
        <div class="order-summary-line total"><span>Total</span><span>MYR ${total.toLocaleString()}</span></div>
      </div>
...
function placeOrder(event) {
  ...
  // Demo only — no backend/payment wired up. To go live, replace this with
  // a real request, e.g.:
  ...
  document.getElementById('checkout-body').innerHTML = `
    <div class="order-confirm">
      ...
      <p>This is a demo checkout, so no real order was placed — wire up placeOrder() in script.js to your backend to go live. Our team would normally follow up within one business day.</p>
      <button class="btn btn-outline" id="checkout-done">Close</button>
    </div>`;
...
function init() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    closeProductModal(); closeCart(); closeCheckout();
  });
